    ...
    >
      <div class="grid-x small-up-1 medium-up-2 large-up-3">
        <div v-for="item in data.results" class="cell card-cell">
          <div class="card">
            <div >
              <router-link :to="{ name: 'FilmId', params: { id: getFilmId(item) }}">
                <img :src="getImage(item.episode_id)" class="equalized">
              </router-link>
            </div>
            ...
            <div class="card-section" >
              <div class="card-top">
                <h4><b>
                  <router-link :to="{ name: 'FilmId', params: { id: getFilmId(item) }}">{{ item.title }}
                  </router-link>
                </b></h4>
                <p>Director: {{item.director}}
                  <br>Produzenten: {{item.producer}}
                  <br>Erscheinungsdatum: {{item.release_date}}
                </p>
              </div>
              <div class="card-bottom">
                <b>
                  <router-link :to="{ name: 'FilmId', params: { id: getFilmId(item) }}">Mehr Informationen....
                  </router-link>
                </b>
              </div>
            </div>
